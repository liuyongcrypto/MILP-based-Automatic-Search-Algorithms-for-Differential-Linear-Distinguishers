...
def Constraint_initialize(R, f, variable):
    ...
    resR = []
    for i in range (32):
        resR.append(GetVariables(R,"X",32,variable)[i])
        resR.append(GetVariables(R,"Y",32,variable)[i])

    if (AAA == BBB):
        f.write(" + ".join(resR) + " = 1 " + "\n")
        if (AAA <= 31):
            f.write(GetVariables(R,"Y",32,variable)[AAA] + " = 1\n")
        else:
            f.write(GetVariables(R,"X",32,variable)[AAA-32] + " = 1\n")
        if (BBB <= 31):
            f.write(GetVariables(R,"Y",32,variable)[BBB] + " = 1\n")
        else:
            f.write(GetVariables(R,"X",32,variable)[BBB-32] + " = 1\n")
    else:
        f.write(" + ".join(resR) + " = 2 " + "\n")
        if (AAA <= 31):
            f.write(GetVariables(R,"Y",32,variable)[AAA] + " = 1\n")
